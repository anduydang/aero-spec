...
import { Cpu, Target, SlidersHorizontal, Globe, Sun, Moon, Sparkles, MousePointerClick } from 'lucide-react';
      ...
      <div className="flex items-center gap-3.5">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs dark:text-slate-200 text-slate-700 dark:bg-slate-800 bg-white px-2.5 py-0.5 rounded-lg border dark:border-slate-700 border-slate-300 hidden md:flex items-center gap-1 font-semibold shadow-sm">
      ...
      <div className="flex flex-wrap items-center gap-2.5">
        ...
        <div className="flex items-center gap-1.5 dark:bg-slate-900 bg-white border dark:border-slate-800 border-slate-300 px-3 py-1.5 rounded-xl shadow-sm">
          ...
          <select 
            ...
            className="bg-transparent text-xs font-bold text-amber-600 dark:text-amber-400 focus:outline-none cursor-pointer"
          >
            <option value="full" className="dark:bg-slate-900 bg-white text-slate-900 dark:text-white">
              {lang === 'EN' ? '🔥 Fully Loaded Rig (RTX 4070 Ti, Dual RAM, 2x NVMe, AIO)' : '🔥 Cấu hình đầy đủ (RTX 4070 Ti, Dual RAM, 2x NVMe, AIO)'}
            </option>
            <option value="missing" className="dark:bg-slate-900 bg-white text-slate-900 dark:text-white">
              {lang === 'EN' ? '⚡ Missing Parts (No dGPU / iGPU, 1x RAM Stick, 1x NVMe)' : '⚡ Khuyết linh kiện (Không GPU rời/iGPU, 1 thanh RAM, 1 NVMe)'}
            </option>
          </select>
        </div>
